...
if (Test-Path $WindowsKitsPath) {
    ...
    Write-Host "⚠️ Warning: Windows SDK path not found: $WindowsKitsPath" -ForegroundColor Yellow
}
...
try {
    ...
    Write-Host "  Found at: $(Get-Command signtool.exe -ErrorAction SilentlyContinue | Select-Object -ExpandProperty Source)" -ForegroundColor Gray
} catch {
...
# Build with electron-builder using temporary config with signtool options
...
if ($exeFiles) {
    ...
    Write-Host "⚠️ No executable found in dist-release folder" -ForegroundColor Yellow
}
