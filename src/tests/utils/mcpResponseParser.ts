...
export interface MCPResponse {
  ...
  result?: MCPResult;
  error?: any;
  content?: MCPResponseContent[]; // Add support for direct content format
}
...
export function extractJsonFromMCPResponse(response: MCPResponse): any {
  ...
  if (response.result?.content && Array.isArray(response.result.content)) {
    ...
    content = (response as any).content[0];
  ...
  try {
    ...
    if (extracted) {
      try {
        ...
        throw new Error(`Failed to parse extracted JSON: ${(parseError as Error).message}`);
      }
    }
    
    throw new Error(`Failed to parse JSON from text: ${(error as Error).message}`);
  }
}
